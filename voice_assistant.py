...
async def stream_audio_to_api(websocket):
    ...
    try:
        while True:
            ...
            # Create the message to send audio as per documentation guidelines
            audio_message = {
                "type": "conversation.item.create",
                "session": session_id,  # Explicitly pass session ID
                "item": {
                    "type": "message",
                    "role": "user",
                    "content": [
...
async def initialize_session(websocket):
    global session_id  # Use the global session ID
    session_update = {
        "type": "session.update",
    ...
    for attempt in range(5):  # Try up to 5 times
        try:
            ...
            if response_data.get("type") == "session.created":
                session_id = response_data["session"]["id"]  # Store the session ID
